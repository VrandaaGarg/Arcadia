  ...
  return (
    <div className="min-h-screen px-4 py-24 flex flex-col items-center bg-[#0B1120] bg-[radial-gradient(ellipse_at_top,#1F2937,#0B1120)] text-white relative">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center animate-fadeIn">
        ...
        {/* Chess Board with Consistent Piece Colors */}
        <div className="p-6 bg-gradient-to-br from-[#8B4513] to-[#654321] rounded-xl border-4 border-[#3D2B1F] shadow-[0_0_30px_rgba(139,69,19,0.3)]">
          <div className="grid grid-cols-8 gap-0.5 p-2 bg-[#3D2B1F]">
                  ...
                  <button
                    ...
                    className={`w-12 h-12 md:w-16 md:h-16 flex items-center justify-center
                      ...
                      ${((rowIndex + colIndex) % 2 === 0)
                        ? 'bg-[#E8D0AA] hover:bg-[#F0D8B2]'
                        : 'bg-[#B58863] hover:bg-[#BD8F6A]'}
                      ${isSelected ? 'ring-4 ring-yellow-400/50' : ''}
                      transition-all duration-300`}
                  >
                    {piece && (
                      <div className={`relative z-10 transform transition-all duration-300 
                        group-hover:scale-110 ${isSelected ? 'scale-110' : ''}`}
                      >
                        {/* Shadow for both white and black pieces */}
                        <span className={`absolute inset-0 blur-[1px] text-4xl md:text-5xl font-bold select-none
                          ${piece.color === 'b' 
                            ? 'text-black/20 translate-y-[2px] translate-x-[2px]' 
                            : 'text-black/20 translate-y-[2px] translate-x-[2px]'}`}
                        >
                          {PIECE_SYMBOLS[piece.color === 'w' ? piece.type.toUpperCase() : piece.type]}
                        </span>
                        
                        {/* Main piece with consistent colors */}
                        <span className={`relative block transform-gpu text-4xl md:text-5xl font-bold select-none
                          ${piece.color === 'w'
                            ? 'text-black [text-shadow:0_2px_4px_rgba(0,0,0,0.8)]' 
                            : 'text-black [text-shadow:0_2px_4px_rgba(0,0,0,0.4)]'}
                    ...
                    {isPossibleMove && !piece && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-3 h-3 rounded-full bg-green-500/40 shadow-[0_0_10px_rgba(34,197,94,0.3)]"></div>
                      </div>
                    )}
                  </button>
                );
          ...
          {/* Board Labels */}
          <div className="flex justify-between px-4 mt-2 text-[#E8D0AA]">
